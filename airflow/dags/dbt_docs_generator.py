...
def verify_redshift_connection():
    ...
    print(
        f"Connecting to Redshift at {conn.host}:{conn.port} with user {conn.login}"
    )
    try:
        psycopg2.connect(
            dbname=conn.schema,
...
def dbt_docs_generator():
    ...
    generate_dbt_docs = DbtDocsOperator(
        ...
        callback=lambda project_dir: save_docs_locally(
            project_dir, "/opt/airflow/dbt-docs"
        ),
        env={"PATH": os.environ["PATH"]},
    )
