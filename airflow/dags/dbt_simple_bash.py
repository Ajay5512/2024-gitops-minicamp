...
def dbt_bash_dag():
    ...
    dbt_run_task = BashOperator(
        ...
        bash_command='source $AIRFLOW_HOME/dbt_venv/bin/activate && cd /$AIRFLOW_HOME/dbt/nexabrands_dbt && dbt run',
    )

    # Define the dbt test command using BashOperator
    dbt_test_task = BashOperator(
        task_id='dbt_test_task',
        bash_command='source $AIRFLOW_HOME/dbt_venv/bin/activate && cd /$AIRFLOW_HOME/dbt/nexabrands_dbt && dbt test',
    )
