...
from pyspark.sql.functions import col, lit, regexp_replace, to_date, trim, upper, when
...
if __name__ == "__main__":
    ...
    s3_output_path = "s3://nexabrands-prod-target/order_lines/"  # Output directory

    # Load and clean data
    order_lines_df = load_order_lines_data(glue_context, s3_input_path)
    cleaned_order_lines = clean_order_lines_data(order_lines_df)

    # Save the cleaned data to S3 as a Parquet file
    cleaned_order_lines.write.mode("overwrite").parquet(s3_output_path)
